...
def dimensionality_reduction(indexes, list_of_sets):
...
def theyre_boring(dataset):
    list_of_variances = []
    dim = len(dataset[0][1])
    output = []

    for i in range(dim):
        list_of_variances.append(pca.var_of_dim(dataset, i))

    mean = helper.mean_of_list(list_of_variances)
    deviation = helper.std_dev(helper.var_of_list(list_of_variances, mean))

    for i in range(dim):
        if list_of_variances[i] / deviation > 1:
            output.append(1)
        else:
            output.append(0)
